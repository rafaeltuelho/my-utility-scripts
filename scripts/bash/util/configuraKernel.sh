...
PERCENTMEM=`echo "scale=2; (10/100) * $MEMTOTAL" | bc | cut -f1 -d.` # nesse caso serah reservado 20% do total para o SO

# Caso a mem. disponivel seja igual ou inferior a 8gb reserva 3g pro SO
if [ $MEMTOTAL -le 8388608 ] ; then
  PERCENTMEM=3145728
elif [ $MEMTOTAL -le 16777216 ] ; then
  PERCENTMEM=`echo "$PERCENTMEM * 3" | bc`
fi

...
doCreateNewJBossGroup() {
     echo " "
     echo -n "Informe nome do Grupo de instâncias JBossEAP: "
     read GROUPNAME

     echo " "
     echo " Plugin: "
     echo "    JBossAS   para JBoss versão <  5.x"
     echo "    JBossAS5  para JBoss versão >= 5.x"

     select OPT in "JBossAS" "JBossAS5"; do

        echo " "
        echo -n "Adicionar recursos filhos (grupo recursivo)? [s|N]"
        read RESPOSTA
...
doAddNewJBossInstanceInGroup() {
     echo " "
     echo -n "Informe nome do Grupo de instâncias JBossEAP: "
     read GROUPNAME

     echo " "
     echo -n "Informe o criterio de pesquisa "
     echo "Ex: tolnedra.belgariad JBoss EAP 4.3.0.GA_CP03 node1 (192.168.100.50:1099)"
     read RESPOSTA
     ...
     $JON_CLI_HOME/bin/rhq-cli.sh \
        $RHQ_OPTS --args-style=named -f \
        $SCRIPT_HOME/JBossGroupDeploy.js \
        operation="addNewJBossInstanceInGroup" \
        groupName=$GROUPNAME \
        searchPattern=$RESPOSTA
}
